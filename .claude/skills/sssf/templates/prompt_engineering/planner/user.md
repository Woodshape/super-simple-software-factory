...
1. Write the full plan to `<context_handoff_dir>/plan.md` — this is the copy the builder reads. Its first bytes must be exactly this canonical frontmatter, followed by the plan Markdown:
   ...
   Use no other frontmatter keys. A plan-only workflow leaves this lifecycle status as `planned`.
...
3. Confirm the copied durable spec and `<context_handoff_dir>/plan.md` are byte-identical, then emit your `Report` JSON declaring BOTH paths in `artifacts`.
...
Both `artifacts` entries are the paths you ACTUALLY wrote, `_v2` suffix and all. Gates open these files, require canonical `status: planned` metadata, and compare their bytes — a name you meant to use or a retyped copy fails them.
