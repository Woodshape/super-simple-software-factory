...
Phases: engineer(request) -> planner -> code(spec_start) -> builder -> code(test) [-> builder(fix) -> code(test) ... bounded] -> code(spec_complete) -> git(commit)
...
def main(prompt: str, config: str = "adws/adw_sssf_config/sssf.config.yaml", adw_id: str | None = None) -> int:
    ...
    if test is not None and test.passed:
        with run.phase(PhaseParams(name="spec_complete", kind="code", owner="specs",
                                   description="Accept the implementation only after the suite passes")) as ph:
